...
import type {
  ...
  ITenantAppointmentRecordDocument,
} from "../../../models/TenantAppointmentRecord.js";
...
import type { ITenantServiceItemDocument } from "../../../models/TenantServiceItem.js";
import TenantClientProfile from "../../../models/TenantClientProfile.js";
import type { ITenantClientProfileDocument } from "../../../models/TenantClientProfile.js";
...
interface ListAppointmentsResult {
  results: ITenantAppointmentRecordDocument[];
...
const normalizeTime = (value: unknown): string =>
  (typeof value === "string" ? value.trim() : "") || "09:00";

const formatTimeFromDate = (date: Date | string): string => {
...
const calculateEndAt = (startAt: Date | string, duration: number): Date => {
...
async function populateAppointmentDoc(
  appointment: ITenantAppointmentRecordDocument | null
): Promise<ITenantAppointmentRecordDocument | null> {
  if (!appointment) {
    return appointment;
  }
...
async function assertStaff({
  ...
  return staff as ITenantStaffDocument;
}
...
async function assertService({
  ...
  return service as ITenantServiceItemDocument;
}
...
async function assertClient({
  ...
  return client as ITenantClientProfileDocument;
}
...
function resolveServiceIds(payload: AppointmentPayload): string[] {
...
function resolveStartAt(payload: AppointmentPayload): Date {
  if (payload.startAt) {
...
export async function listAppointments({
  ...
  to,
  pagination = {},
}: ListAppointmentsParams): Promise<ListAppointmentsResult> {
  ...
  if (from || to) {
    ...
    if (from) {
      startAtFilter.$gte = new Date(from);
    }
    if (to) {
      startAtFilter.$lte = new Date(to);
    }
    baseQuery.startAt = startAtFilter;
  }
  ...
  return {
    results: appointments as ITenantAppointmentRecordDocument[],
...
export async function createAppointment({
...
}): Promise<ITenantAppointmentRecordDocument> {
  ...
  await populateAppointmentDoc(
    appointment as ITenantAppointmentRecordDocument
  );
  return appointment as ITenantAppointmentRecordDocument;
}
...
export async function updateAppointment({
  ...
  if (payload.branchId) {
    ...
    appointment.branch = branch._id as Types.ObjectId;
  }

  if (payload.clientId) {
    const client = await assertClient({ tenantId, clientId: payload.clientId });
    appointment.client = client._id as Types.ObjectId;
  ...
  if (payload.staffId) {
    ...
    appointment.staff = staff._id as Types.ObjectId;
  ...
  if (payload.serviceIds || payload.serviceId) {
    ...
    appointment.services = services.map((service) => ({
      service: service._id as Types.ObjectId,
      name: service.name,
      duration: service.duration,
      price: service.price,
    }));
    appointment.service = services[0]._id as Types.ObjectId;
  ...
  if (payload.endAt) {
    ...
    appointment.endAt = calculateEndAt(
      appointment.startAt,
