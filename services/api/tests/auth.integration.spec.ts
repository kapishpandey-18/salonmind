...
const buildSendOtp = (surface: string) => `/v1/auth/${surface}/otp/send`;
const buildResendOtp = (surface: string) => `/v1/auth/${surface}/otp/resend`;
const buildVerifyOtp = (surface: string) => `/v1/auth/${surface}/otp/verify`;
const buildRefresh = (surface: string) => `/v1/auth/${surface}/token/refresh`;
...
interface AuthData {
  accessToken: string;
  refreshToken: string;
  user: { role: string; tenant?: string };
}

const completeOtpFlow = async (
  surface: string,
  phone: string,
...
describe("Auth module - OTP + token flows", () => {
  ...
  test("Access token payload includes ObjectId sessionId", async () => {
    await seedOwnerUser();
    const data = await completeOtpFlow("salon-owner", OWNER_PHONE);
    const payload = jwt.decode(data.accessToken) as { sessionId: string; surface: string };
  ...
  test("Refresh token returns new access token with sessionId string", async () => {
    ...
    expect(refreshRes.status).toBe(200);
    const payload = jwt.decode(refreshRes.body.data.accessToken) as { sessionId: string };
    expect(payload.sessionId).toMatch(SESSION_ID_REGEX);
  });
  ...
  test("Invalid sessionId in token is rejected with 401", async () => {
    await seedOwnerUser();
    const data = await completeOtpFlow("salon-owner", OWNER_PHONE);
    const original = jwt.decode(data.accessToken) as { sub: string; surface: string };
