import ms from "ms";
import type { Surface, EnvConfig, SurfaceConfig } from "../types/index.js";
...
const parseDuration = (value: string, key: string): number => {
  const duration = ms(value as ms.StringValue);
  if (!duration) {
    throw new Error(
      `Invalid duration for ${key}. Use values like "15m" or "7d".`
    );
  }
  return duration;
};

const surfaceConfig = {} as Record<Surface, SurfaceConfig>;
...
const env: EnvConfig = {
  ...
  adminAllowlist: process.env
    .ADMIN_ALLOWLIST_NUMBERS!.split(",")
