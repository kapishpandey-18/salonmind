...
export const LoginPage = () => {
  ...
  const { sendOtp, resendOtp, verifyOtp, isAuthenticated, sessionExpired } = useAuth();
  ...
  const handleVerify = async (event: FormEvent) => {
    event.preventDefault();
    if (!challengeId) {
      setError("Request an OTP before verifying");
      return;
    }
    setIsVerifying(true);
    setError(null);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-brand-500/10 to-white px-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl">
        <div className="mb-6">
          <h1 className="text-2xl font-semibold text-slate-900">SalonMind Admin</h1>
          <p className="text-sm text-slate-500">Secure OTP login for platform administrators.</p>
          {sessionExpired && (
        ...
        <form className="space-y-4" onSubmit={challengeId ? handleVerify : handleSendOtp}>
          <label className="block text-sm font-medium text-slate-600">
            ...
            <input
              ...
              className="mt-1 w-full rounded-lg border border-slate-200 px-4 py-2 focus:border-brand-500 focus:outline-none"
          ...
          {challengeId && (
            <label className="block text-sm font-medium text-slate-600">
              ...
              <input
                ...
                className="mt-1 tracking-widest text-center text-lg w-full rounded-lg border border-slate-200 px-4 py-2 focus:border-brand-500 focus:outline-none"
